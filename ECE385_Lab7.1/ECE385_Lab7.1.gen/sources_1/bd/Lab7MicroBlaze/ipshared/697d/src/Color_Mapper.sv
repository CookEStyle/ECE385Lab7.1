...
//        inv = glyph[7]; //Inversion bit
        ...
        f_index = (f_glyph / 2);
        if(f_glyph[0]) begin
            f_red = palette[f_index][28:25];
            f_green = palette[f_index][24:21];
            f_green = palette[f_index][20:17];
        end else begin    
            f_red = palette[f_index][12:9];
            f_green = palette[f_index][8:5];
            f_green = palette[f_index][4:1];
        end
        
        b_index = (b_glyph / 2);
        if(b_glyph[0]) begin
            b_red = palette[b_index][28:25];
            b_green = palette[b_index][24:21];
            b_green = palette[b_index][20:17];
        end else begin    
            b_red = palette[b_index][12:9];
            b_green = palette[b_index][8:5];
            b_green = palette[b_index][4:1];
        end
